...
# 카카오 로컬 API - 반경 내 키워드 시설 개수 카운트
def get_kakao_count(lat, lon, keyword, radius=1000):
    headers = {"Authorization": f"KakaoAK {KAKAO_API_KEY}"}
    params = {
        "query": keyword,
        "x": lon,
...
# 카카오 로컬 API - 반경 내 최근접 시설까지 거리 반환 (미터 단위)
def get_kakao_nearest(lat, lon, keyword, radius=5000):
    headers = {"Authorization": f"KakaoAK {KAKAO_API_KEY}"}
    params = {
        "query": keyword,
        "x": lon,
    ...
    try:
        ...
        result = response.json()
        if result["documents"]:
            return float(result["documents"][0]["distance"])
    except Exception as e:
        print(f"오류 발생: {keyword} -> {e}")
    return None